...
This chapter aims to describe all the different methodologies used to analyze the data collected during the previous phase, and explained in details in \Cref{cha:data}. 
...
\begin{figure}[H]
	\centering
	\begin{tikzpicture}
		\filldraw (-2,2) node[RectObject, inner xsep=1cm, dashed](pandemic){pandemic};
		\draw (-2,3) node[RectObject, inner xsep=1cm, draw=gray](hospital){hospital};
...
Given the fact that the information produced by m3inference (see \Cref{subsec:m3inference} for further details) not only regards the gender of a user, we also thought about considering the emotions expressed by a particular age bracket.
...
In practice, if we take \Cref{lst:nominatim-geocode} as example, and we decide to map all the users from Italy based on their county, all the ones with “Milano, Lombardia” as their original location will then be assigned to “Milano”. Instead, all the users that have a country different from “Italy”, or they do not have a county, will be discarded.