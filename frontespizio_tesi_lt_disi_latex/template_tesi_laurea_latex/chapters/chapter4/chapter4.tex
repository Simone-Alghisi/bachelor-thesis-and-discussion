...
\begin{figure}[H]
	\centering
    	\includegraphics[scale=.35]{en_liwc_categories.svg}
    	\caption{Proportion of weekly users per LIWC category in the English tweets}
    	\label{fig:en-liwc-category}
\end{figure}

As discussed in \Cref{sec:liwc}, we decided to use LIWC in order to validate the results obtained from EmoLex. \Cref{fig:en-liwc-category} shows the proportion of users that expressed a certain LIWC category (selected from the possible categories identified).
